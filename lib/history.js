...
function loadHistoryFromDisk() {
  ...
  // On vide l'objet actuel et on injecte les données chargées pour garder la même référence en mémoire
  history.days = parsed.days || {};
  history.summaries = parsed.summaries || {};
  history.version = parsed.version || 2;
  console.log(`[history] 📁 Historique restauré (${Object.keys(history.days).length} jours)`);
}
