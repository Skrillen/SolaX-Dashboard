...
/* --------------------------------------------------------------------------
   ...
   -------------------------------------------------------------------------- */
const cache = { _meta: { lastResetDay: new Date().getDate() } };
...
function checkMidnightReset() {
  ...
  if (currentDay !== lastResetDay) {
    ...
    cache._meta.lastResetDay = currentDay;
...
function loadCacheFromDisk() {
  ...
  if (!parsed) return;
  for (const sn of [...wifiSns, "meter", "_meta"]) {
    const entry = parsed[sn];
    if (sn === "_meta" && entry) {
      cache._meta = entry;
      if (entry.lastResetDay) lastResetDay = entry.lastResetDay;
      continue;
    }
    if (
      entry &&
    ...
    if (errCode === 10000 && Array.isArray(response.data.result)) {
      ...
      for (const item of response.data.result) {
        ...
        const mappedData = {
          ...
          powerdc1:      item.mpptMap?.MPPT1Power ?? 0,
          powerdc2:      item.mpptMap?.MPPT2Power ?? 0,
        };
