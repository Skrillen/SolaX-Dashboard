...
function checkMidnightReset() {
  ...
  if (currentDay !== lastResetDay) {
    ...
    console.log(`[${timeStr}] 🕛 Minuit détecté : Reset de la production journalière.`);
    ...
    // Capture du nouveau baseline pour le Meter
    if (cache["meter"]?.data) {
    ...
    if (errCode === 10000 && Array.isArray(response.data.result)) {
      ...
      for (const item of response.data.result) {
        if (item.deviceSn === METER_SN) {
          const currentImport = item.importEnergy || 0;
          const currentExport = item.exportEnergy || 0;
